...
```{r}
model_test <- lm(SaleType ~., data=data.test)
pred_test <- model_test$fitted.values
tally_table <- data.frame(actual=data.test$SaleType, predicted=pred_test)
mape_test <- mean(abs(tally_table$actual-tally_table$predicted)/tally_table$actual)
